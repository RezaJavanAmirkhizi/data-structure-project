...
export default function Exam1(props) {
    ...
    useEffect(() => {
        ...
        let questions = [];
        for (let i in exam1W) {
            questions.push(exam1W[i].english)
        }
        ...
        for (let i = wrongNumbers; i < props.questionWords.length; i++) {
            if (counter.allCount === 10) {
                ...
                if (props.questionWords[i].persian !== exam1W[counter.allCount].persian) {
                    if (value.length < 3) {
        ...
        value.splice(correctNumber, 0, exam1W[counter.allCount].persian) //adds the wanted choise to the wanted index in the array
    ...
    const checkQuestion = (question) => {
        ...
        if (question === choices.allChoises[choices.correctChoice]) {
            if (exam1W[counter.allCount].level === 0) {
                exam1W[counter.allCount].level++;
                exam1W[counter.allCount].nextDate = new moment().add(3, "days").format('YYYY-MM-DD');
            }
            else if (exam1W[counter.allCount].level === 1) {
                exam1W[counter.allCount].level++;
                exam1W[counter.allCount].nextDate = new moment().add(5, "days").format('YYYY-MM-DD');
            }
            else {
                exam1W[counter.allCount].level++;
                exam1W[counter.allCount].nextDate = new moment().add(15, "days").format('YYYY-MM-DD');
            }
        }
        else {
            exam1W[counter.allCount].nextDate = new moment().add(1, "days").format('YYYY-MM-DD');
        }
