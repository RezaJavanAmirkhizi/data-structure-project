	...
	useEffect(() => {
		...
		for (let i = wrongNumbers; i < props.questionWords.length; i++) {
			if (counter.allCount === 10) {
				...
				if (
					props.questionWords[i].persian !==
					exam1W[counter.allCount].persian
				) {
					if (value.length < 3) {
	...
	const checkQuestion = (question) => {
		if (question === choices.allChoises[choices.correctChoice]) {
			if (exam1W[counter.allCount].level === 0) {
				...
				exam1W[counter.allCount].nextDate = new moment()
					.add(3, "days")
					.format("YYYY-MM-DD");
			...
			{counter.allCount === exam1W.length ? (
				<div className="result">
					...
					<span style={{ color: "red" }}>
						تعداد جواب های غلط :{" "}
						{counter.allCount - counter.correctCounter}
					</span>
					<span style={{ color: "white" }}>
						درصد :{" "}
						{(
							(counter.correctCounter / counter.allCount) *
							100
						).toFixed(0)}{" "}
						%
					</span>
				...
				<div className="questionContainer">
					<div className="question">
						<span>{question[counter.allCount]}</span>
					</div>
					<hr />
					...
					<h3
						style={{ color: message === "درست" ? "green" : "red" }}
					>
						{message}
					</h3>
				</div>
